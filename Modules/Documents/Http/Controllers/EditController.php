  ...
  public function index(Request $request)
  {
    ...
    if(in_array($type,['khs', 'turnkey','surat_pengikatan','sp','amandemen_sp','adendum','side_letter'])){
      if(isset($dt->latar_belakang_kesanggupan_mitra[0])){
        ...
        $dt->lt_tanggal_kesanggupan_mitra   = Helpers::date_set($dt->latar_belakang_kesanggupan_mitra[0]['meta_desc']);
    ...
    if(in_array($type,['surat_pengikatan'])){
      if(isset($dt->latar_belakang_rks[0])){
        ...
        $dt->lt_tanggal_rks  = Helpers::date_set($dt->latar_belakang_rks[0]['meta_desc']);
    ...
    if(count($dt->latar_belakang_optional)>0){
      foreach($dt->latar_belakang_optional as $key => $val){
        ...
        $lt_optional['f_latar_belakang_tanggal'][$key]  = Helpers::date_set($val->meta_title);
    ...
    if(count($dt->asuransi)>0){
      foreach($dt->asuransi as $key => $val){
        ...
        $jas['doc_jaminan_startdate'][$key] = Helpers::date_set($val->doc_jaminan_startdate);
        $jas['doc_jaminan_enddate'][$key]   = Helpers::date_set($val->doc_jaminan_enddate);
    ...
    $dt->doc_startdate= Helpers::date_set($dt->doc_startdate);
    $dt->doc_enddate= Helpers::date_set($dt->doc_enddate);
    $data['page_title'] = 'Edit Dokumen';
    $data['doc_type'] = $dt->jenis->type;

    if($user_type!='subsidiary'){
      if(isset($dt->pemilik_kontrak->meta_name)){
  ...
  public function store_ajax(Request $request)
  {
    ...
    $validator->after(function ($validator) use ($request) {
      ...
      if(
          ...
          $validator->errors()->has('doc_template_id') ||
