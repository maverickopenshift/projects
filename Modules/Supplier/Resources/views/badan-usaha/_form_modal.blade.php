        ...
        <div class="modal-content">
            <form id="form-me" action="#" method="post">
                ...
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Edit Badan Usaha</h4>
                </div>
...
<script>
    $(function() {
        ...
        formModal.on('show.bs.modal', function (event) {
            ...
            modal.find('.modal-title').text(title+' Badan Usaha')
            if(title=='Edit'){
                var data = button.data('data');
                //data = JSON.parse(data);
                modal.find('.modal-body input#id').val(data.id)
                modal.find('.modal-body input#text').val(data.text)
                modal.find('form').attr('action','{!! route('supplier.badanusaha.update') !!}')
            }
            else{
                modal.find('.modal-body input#text').val('')
                modal.find('form').attr('action','{!! route('supplier.badanusaha.add') !!}')
            }
        })
        ...
        $(document).on('click','.btn-delete',function (event) {
            ...
            $.ajax({
                url: '{!! route('supplier.badanusaha.delete') !!}',
                type: 'delete',
                chache:false,
