    ...
    public function data()
    {
     ...
     public function tambah()
     {
        ...
        if($sql){
          ...
          $data['action_url']   = route('usersupplier.update_ajax');
          $data['action_type']  = "edit";

          $sql->asset = $sql->asset+0;
          $id=$sql->id;

          $sql->akte_awal_tg     = Helpers::date_set($sql->akte_awal_tg);
          $sql->akte_akhir_tg    = Helpers::date_set($sql->akte_akhir_tg);
          $sql->siup_tg_terbit   = Helpers::date_set($sql->siup_tg_terbit);
          $sql->siup_tg_expired  = Helpers::date_set($sql->siup_tg_expired);
          $sql->npwp_tg          = Helpers::date_set($sql->npwp_tg);
          $sql->tdp_tg_terbit    = Helpers::date_set($sql->tdp_tg_terbit);
          $sql->tdp_tg_expired   = Helpers::date_set($sql->tdp_tg_expired);
          $sql->idp_tg_terbit    = Helpers::date_set($sql->idp_tg_terbit);
          $sql->idp_tg_expired   = Helpers::date_set($sql->idp_tg_expired);
          ...
          $sql->klasifikasi_text = $klasifikasi_text;
          $sql->klasifikasi_kode = $klasifikasi_kode;
          ...
          $sql->legal_dokumen_nama = $legal_dokumen_nama;
          ...
          $data['action_url']   = route('supplier.insert_ajax');
    ...
    if ($validator->fails ()){
      ...
      foreach($request->iujk_no as $l => $val){
        ...
        if(isset($request['file_sertifikat_dokumen'][$l])){
          ...
          $fileName   = Helpers::set_filename($data->kd_vendor,$request['nama_sertifikat_dokumen'][$l]);
          $request['file_sertifikat_dokumen'][$l]->storeAs('supplier/sertifikat_dokumen', $fileName);
        }
        ...
        foreach($new_dokumen_up as $key => $val){
          if(!empty($val)
            ...
            if(is_object($val)){
              ...
              //$fileName   = $usr_nm.'_'.str_slug($request->legal_dokumen_nama[$key]).'_'.time().'.pdf';
              //$fileName   = $data->kd_vendor.'_'.str_slug($nama).'_'.time().'.pdf';
              $fileName   = Helpers::set_filename($data->kd_vendor,$request['legal_dokumen_nama'][$key]);
        ...
        foreach($new_dokumen_up as $key => $val){
          if(!empty($val)
            ...
            if(is_object($val)){
              $file       = $request['legal_dokumen'][$key];
              $fileName   = Helpers::set_filename($data->kd_vendor,$request->legal_dokumen_nama[$key]);
              //$fileName   = $data->kd_vendor.'_'.str_slug($request->legal_dokumen_nama[$key]).'_'.time().'.pdf';
              //return $kd_vendor.'_'.str_slug($name).'_'.time().'_'.str_random(5).'.pdf';

              $file->storeAs('supplier/legal_dokumen', $fileName);              
            }else{
              $fileName = $new_dokumen_up[$key];
            }

            $mt_data->object_value = json_encode([
              'name'  => $request->legal_dokumen_nama[$key],
              'file'  => $fileName]);
            $mt_data->save();
          }
        }
